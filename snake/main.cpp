...
int main(int, char **)
{
	...
	SDL_SetColorKey(img, 1, colorkey);
	SDL_Window *win = SDL_CreateWindow("game", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 640, 480, SDL_WINDOW_SHOWN);
	SDL_Renderer *ren = SDL_CreateRenderer(win, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
	...
	dest.w = dest.h = 200;
	SDL_RenderClear(ren);
	SDL_RenderCopy(ren, background, 0, 0);
